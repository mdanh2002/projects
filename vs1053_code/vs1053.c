...
void vs1053_WriteRegister(unsigned char addressbyte, unsigned char highbyte, unsigned char lowbyte)
{
  ...
  MP3_XCS = 0; //Select control

  //SCI consists of instruction byte, address byte, and 16-bit data word.
  spi2Write(0x02); //Write instruction
  spi2Write(addressbyte);
  spi2Write(highbyte);
  spi2Write(lowbyte);
  while(!MP3_DREQ_IN) ; //Wait for DREQ to go high indicating command is complete
  
  MP3_XCS = 1; //Deselect Control
}
...
unsigned int vs1053_ReadRegister (unsigned char addressbyte)
{
  ...
  //SCI consists of instruction byte, address byte, and 16-bit data word.
  spi2Write(0x03); //Read instruction
  spi2Write(addressbyte);

  char response1 = spi2Write(0xFF); //Read the first byte
  while(!MP3_DREQ_IN) ; //Wait for DREQ to go high indicating command is complete
  char response2 = spi2Write(0xFF); //Read the second byte
  while(!MP3_DREQ_IN) ; //Wait for DREQ to go high indicating command is complete
...
VS1053_INFO vs1053_setup()
{
  ...
  MP3_RESET = 0; // hardware reset
  spi2Write(0xFF); // dummy byte
  delay_ms(10);
  MP3_RESET = 1; // bring up VS1053
...
void vs1053_play(unsigned char* data, unsigned int len){
	...
	MP3_XDCS = 0; //Select Data
	for (i = 0 ; i < 2048 ; i++) {
		while(!MP3_DREQ_IN); //If we ever see DREQ low, then we wait here
		spi2Write(0);
	}
