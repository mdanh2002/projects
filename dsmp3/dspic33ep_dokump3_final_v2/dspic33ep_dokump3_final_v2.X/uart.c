...
void InitUART(unsigned char highClock)
{
// **************************
// (1)Define U1MODE
// **************************
...
// **************************
    ...
    // not yet, will be enabled as needed
    // U1MODEbits.UARTEN = 1;        // UART modudle
    // U1STAbits.UTXEN = 1;          // Transmit enable
}

void UpdateUART_BRG(unsigned char highClock)
{
    // **************************
    // (2)Define U1BRG
    //     BAUDRATE(Register) = SYSCLK / 32 / BAUDRATE - 1
    // **************************
    U1BRG = ( (highClock ? FOSC_HIGH : FOSC_LOW) / 32 / UART1_SPEED - 1);       
}

void WriteUART(char in_c)
{
    unsigned int c = 0;
    while(U1STAbits.UTXBF != 0 && c < 60000)
    {
        c++;
    }
    U1TXREG = in_c;
