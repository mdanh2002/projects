...
__prog__ static U32 const __attribute__((section("bitrate_table"), space(prog))) bitrate_table[5][15] =
{
    /* MPEG-1 */
...
MAD_RETVAL mad_header_decode (MAD_HEADER *header, MAD_STREAM *stream)
{
  ...
  sync:
    ...
    // TODO; MD added this code to stop processing if the layer is not 3 (the library only supports layer 3 anyway)
    // In addition, if we are trying to seek within an MP3 frame, the sync word search might incorrectly lock on to random data as frame
    // if this happens, the layer value will contain invalid data (e.g. large value) that causes memory fault layer as layer info is looked up
    // we will work around this by ignoring non-layer 3 header!
    if (header->layer != 3)
        goto fail;
