 ...
 **************************************************************************************************/
/*
 Variables are placed into EDS using the space or address attribute.
 
 The __eds__ qualifier is used in conjunction with the EDS or address attribute.
 The compiler will ensure the appropriate values are loaded in DSRPAG and PSWPAG 
 before accessing variables defined with __eds__ qualifier.

__attribute ((eds)) causes a variable to be placed in the EDS at an address selected by the compiler.
*/
MAD_SYNC  decoder_data;
U08        __attribute__((space(xmemory))) stream_data[MAD_BUFFER_MDLEN];
MAD_FIXED  __attribute__((space(xmemory))) frame_data[2][32][18];
