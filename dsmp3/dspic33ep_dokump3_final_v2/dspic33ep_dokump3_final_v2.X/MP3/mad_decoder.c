...
MAD_RETVAL mad_run_async_begin(MAD_DECODER *decoder)
{
    MAD_STREAM *stream;
    MAD_FRAME  *frame;
    MAD_SYNTH  *synth;
    ...
    decoder->sync = (MAD_SYNC*)mad_decoder_alloc(sizeof(MAD_SYNC));
    if (decoder->sync == 0)
        return (RET_ERR);
...
MAD_ASYNC_STATUS mad_run_async_step(MAD_DECODER *decoder, BOOL needData)
{
    ...
    if (needData)
    {
        switch (decoder->input_func(decoder->cb_data, stream))
        {
            ...
            case MAD_FLOW_IGNORE:   goto onStreamContinue;
    ...
    onStreamContinue:
        return ASYNC_CONTINUE_BUFFER;
    
    onStreamError:
        if (stream->error != MAD_ERROR_BUFLEN)
        {
            return ASYNC_COMPLETE_ERROR;
        }
        else {
            return ASYNC_CONTINUE_NEED_DATA;
        }
...
MAD_RETVAL mad_run_async_end(MAD_DECODER *decoder)
{   
