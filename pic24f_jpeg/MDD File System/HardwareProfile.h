...
// Sample clock speed for a 16-bit processor
	...
	/*
		...
		Sd card init in the lib is done by first opening at 400kHz SPI. If successful, SPI clock
		will be adjusted to the maximum possible speed by using:

		OpenSPIM(SYNC_MODE_FAST);

		where SYNC_MODE_FAST is 0x3E (bit5 - bit0) of SPI1CON1 register.

		Bit 5=1 - SPI Master Mode
		Bit 4-2=111 - Sec. Prescaler 1:1
		Bit 1-0=10 - Pri. Precaler 4:1

		The maximum theoretical speed of the PIC24HJ128GP202 is 40 MIPS (80MHz) in normal operating conditions.
		However, it can be over clocked to ~100MHz. SD Card SPI speed supports up to 25MHz.

		Hence, to run the PIC at such a high speed, either change te value of SYNC_MODE_FAST in SD-SPI.h
		to reduce the SPI speed or choose a suitable procesor speed that the SD card can still support.

		The maximum reliable speed that can be used on this PIC on a breadboad is 32MHz. 
		At 80MHz, SD card can be unreliable as the SPI operates at 20MHz, too much for breadboad stray capacitance.
	*/
...
/* SD Card definitions: Change these to fit your application when using
   an SD-card-based physical layer                                   */
	...
	//PM
		...
		#if defined (__PIC24HJ128GP202__) //MD modifies here to use this chip

			// need additional code to remap SPI pin to the pin below
			// http://www.microchip.com/forums/tm.aspx?m=474137&mpage=1&key=PIC24FJ64GA002%2CFAT16&
