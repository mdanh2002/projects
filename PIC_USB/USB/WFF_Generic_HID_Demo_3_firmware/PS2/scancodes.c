...
// refer to http://www.philipstorr.id.au/pcbook/book3/scancode.htm
const rom unsigned char scan_to_ascii[][5] = 
{
...
unsigned char decodeScanCode( unsigned char scanCode, unsigned char flags )
{
	...
	if ( min <= max )
	{
		...
		result = scan_to_ascii[mid][offset];
	}
