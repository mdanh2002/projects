...
#include "shared/8086tiny_interface.h"
#include "shared/midi.h"
...
// not for IRQ!
// 0 = Division by Zero, 1 = Single Step, 2 = Nonmaskable, 3 = Breakpoint, 4 = Overflow, 5 = Print screen, 8 = System timer
// https://docs.huihoo.com/gnu_linux/own_os/appendix-bios_interrupt_1.htm
/*
...
#endif
{
...
#endif
	Interface.Initialise(mem, io_ports);
	...
	while (!ExitEmulation)
	{
		...
		switch (xlat_opcode_id)
		{
			...
			io_ports[scratch_uint] = Interface.ReadPort(scratch_uint, i_w, false);
			if (i_w)
			{
				io_ports[scratch_uint + 1] = Interface.ReadPort(scratch_uint + 1, i_w, true);
			}
			R_M_OP(regs8[REG_AL], =, io_ports[scratch_uint]);
			OPCODE 22: // OUT DX/imm8, AL/AX
			scratch_uint = extra ? regs16[REG_DX] : (unsigned char)i_data0;
			R_M_OP(io_ports[scratch_uint], =, regs8[REG_AL]);
			Interface.WritePort(scratch_uint, io_ports[scratch_uint], i_w, false);
			if (i_w)
			{
				Interface.WritePort(scratch_uint + 1, io_ports[scratch_uint + 1], i_w, true);
			}
			OPCODE 23: // REPxx
			rep_override_en = 2;
			rep_mode = i_w;
			...
			for (scratch_uint = rep_override_en ? regs16[REG_CX] : 1; scratch_uint; scratch_uint--)
			{
				io_ports[scratch2_uint] = Interface.ReadPort(scratch2_uint, i_w, false);
				if (i_w)
				{
					io_ports[scratch2_uint + 1] = Interface.ReadPort(scratch2_uint + 1, i_w, true);
				}

				/*
				19 Aug 2019
				By ToughDev
				There is a small but serious typo in this location in the original code from https://jaybertsoftware.weebly.com/8086-tiny-plus.html
				Original code is:  R_M_OP(mem[SEGREG(REG_ES, REG_DI)], =, io_ports[scratch_uint]);
				This causes the INSW instruction to always read 0 since ES:DI is copied fron io_ports[scratch_uint] and not io_ports[scratch2_uint]

				This bug is only relevant in apps that detects the 80186 CPU and uses REP INSW instruction - everything read will be 0
				for example: NE2000 16-bit packet driver, UE2000.DOS (an NE2000 NDIS driver) and Microsoft NE2000.DOS driver
			...
			for (scratch_uint = rep_override_en ? regs16[REG_CX] : 1; scratch_uint; scratch_uint--)
			{
				...
				Interface.WritePort(scratch2_uint, io_ports[scratch2_uint], i_w, false);
				if (i_w)
				{
					Interface.WritePort(scratch2_uint + 1, io_ports[scratch2_uint + 1], i_w, true);
				}
				INDEX_INC(REG_SI);
			}
		...
		// help us here.
		if (DEBUG_IsPendingIpChange())
		{
			unsigned short newIP = DEBUG_GetNewIPVal();
			printf("Manual instruction pointer set: %04X\n", newIP);
			reg_ip = newIP;
			DEBUG_ClearPendingIpChange();
		}
		else
		{
