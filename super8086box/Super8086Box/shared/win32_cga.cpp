...
static BITMAPINFO GFXFullBmi;
static char *GFXFullBits = (char *)NULL;
...
static int FullScreenAdjustX = 0;
static int FullScreenAdjustY = 0;
static bool FullScreenFit = true;
static bool FullScreen = false;
static int FSMethod = 0;
...
bool needClearSurface = false;
static double FullScalingX = 1;
static double FullScalingY = 1;
static int FullScalingXOffset = 0;
static int FullScalingYOffset = 0;
static int NormalScale = 1;

// =============================================================================
// Local Functions
//
void CGA_SetNeedClearDisplaySurface()
{
	needClearSurface = true;
}

void CGA_ClearDisplaySurface(HWND hwnd)
{	
	HDC hdc = GetDC(hwnd);
	...
	BITMAPINFO GFXFullSurface;
	GFXFullSurface.bmiHeader.biSize = sizeof(BITMAPINFOHEADER);
	GFXFullSurface.bmiHeader.biWidth = fullWidth;
	GFXFullSurface.bmiHeader.biHeight = -fullHeight;
	GFXFullSurface.bmiHeader.biPlanes = 1;
	GFXFullSurface.bmiHeader.biBitCount = 24;
	GFXFullSurface.bmiHeader.biCompression = BI_RGB;
	GFXFullSurface.bmiHeader.biSizeImage = -GFXFullSurface.bmiHeader.biWidth * GFXFullSurface.bmiHeader.biHeight * 3;
	GFXFullSurface.bmiHeader.biXPelsPerMeter = 4096;
	GFXFullSurface.bmiHeader.biYPelsPerMeter = 4096;
	GFXFullSurface.bmiHeader.biClrUsed = 0;
	GFXFullSurface.bmiHeader.biClrImportant = 0;

	char* GFXDisplaySurface = new char[GFXFullSurface.bmiHeader.biSizeImage];
	memset(GFXDisplaySurface, 0, GFXFullSurface.bmiHeader.biSizeImage);
	...
	delete[] GFXDisplaySurface;

	ReleaseDC(hwnd, hdc);
}

static void adjustCursorForFullscreen(RECT &crect)
{
	// crect.bottom = crect.top + (crect.bottom - crect.top) / 1.6;
	// crect.right = crect.left + (crect.right - crect.left) / 1.6;
	crect.left = crect.left + FullScalingXOffset / 2;
	crect.right = crect.right + FullScalingXOffset / 2;
	crect.top = crect.top + FullScalingYOffset / 2;
	crect.bottom = crect.bottom + FullScalingYOffset / 2;
}
...
void prepareFullScreenBuffer(char *src, int rectWidth, int rectHeight, int srcWidth, int dstWidth, int dstHeight) {
	double scaleX = (double)dstWidth / (double)rectWidth;
	double scaleY = (double)dstHeight / (double)rectHeight;

	// integer operations are faster!
	int K = 100;
	int scaleY_K = scaleY * K;
	int scaleX_K = scaleX * K;
	int srcWidth3 = srcWidth * 3;

	char* pixel = (char*)(GFXFullBits);
	for (int y = 0; y < dstHeight; y++)
	{
		int tmp = (y * K / scaleY_K) * srcWidth3;
		for (int x = 0; x < dstWidth; x++)
		{
			int ind = tmp + (x * K / scaleX_K) * 3;
...
void displayFrameOnScreen(HDC hdc, int rectX, int rectY, int rectWidth, int rectHeight, int dstWidth, int dstHeight, BITMAPINFO srcBmi, char* srcBits, bool isVGA8_DrawCO80)
{
	// Default StretchDIBits - 1ms
	// Fast Tone - 10ms
	// Manual - 27ms
	// DWORD start = timeGetTime();

	// https://www.reddit.com/r/HandmadeHero/comments/2n8r6b/fix_for_the_scaling_artifacts_caused_by/
	// when scaling is non-integer, StretchDIBits will be imprecise causing artifacts between frame. 
	// We can use SetStretchBltMode(hdc, STRETCH_HALFTONE); but this is too slow, game will be unplayable
	// In our code we simply perform the stretch manually in prepareFullScreenBuffer(), which will be faster!
	if (FullScreen && FSMethod == 2) // internal stretch
	{
		prepareFullScreenBuffer(srcBits, rectWidth, rectHeight, srcBmi.bmiHeader.biWidth,
			dstWidth * FullScalingX, dstHeight * FullScalingY);
		 
		SetDIBitsToDevice(
			hdc,
			FullScalingXOffset / 2, FullScalingYOffset / 2, dstWidth * FullScalingX, dstHeight * FullScalingY,
			0, 0, 0, dstHeight * FullScalingY,
			GFXFullBits,
			&GFXFullBmi,
			DIB_RGB_COLORS);
	}
	else {
		if (FullScreen && FSMethod == 1) // half tone
		{
		...
		if (!FullScreen && isVGA8_DrawCO80 && NormalScale == 1)
		{
			// fastest way for default VGA 80x25
			SetDIBitsToDevice(
				hdc,
			...
			StretchDIBits(
				hdc,
				FullScreen ? FullScalingXOffset / 2 : 0,
				FullScreen ? FullScalingYOffset / 2 : 0,
				dstWidth * FullScalingX + (FullScreen ? FullScreenAdjustX : 0),
				dstHeight * FullScalingY + (FullScreen ? FullScreenAdjustY : 0),
...
static void CGA_DrawCO40(HWND hwnd, unsigned char *mem)
{
	...
	displayFrameOnScreen(hdc, 0, 0, 320, 200, 640, 400, GFX320bmi, GFX320Bits, false);
	...
	{
		RECT crect;
		crect.top = ((CursorLocation / 40) * 16 + (CRTRegister[0xA] & 0x1f) * 2) * FullScalingY;
		crect.left = (CursorLocation % 40) * 16 * FullScalingX;
		crect.bottom = ((CursorLocation / 40) * 16 + (CRTRegister[0xB] & 0x1F) * 2 + 2) * FullScalingY;
		crect.right = crect.left + (16 * FullScalingX);

		if (FullScreen) // reduce height and width by half, otherwise cursor is very thick!
		{
			adjustCursorForFullscreen(crect);
		}
...
static void VGA8_DrawCO40(HWND hwnd, unsigned char *mem)
{
	...
	displayFrameOnScreen(hdc, 0, 80, 320, 400, 640, 400, GFX640x480bmi, GFX640x480Bits, false);
...
static void CGA_DrawCO80(HWND hwnd, unsigned char *mem)
{
	...
	HDC hdc = GetDC(hwnd);
	// SetStretchBltMode(hdc, STRETCH_BLT_FLAG);
	...
	displayFrameOnScreen(hdc, 0, 0, 640, 200, 640, 400, GFX640bmi, GFX640Bits, false);	
	...
	{
		RECT crect;
		crect.top = ((CursorLocation / 80) * 16 + (CRTRegister[0xA] & 0x1f) * 2) * FullScalingY;
		crect.left = (CursorLocation % 80) * 8 * FullScalingX;
		crect.bottom = ((CursorLocation / 80) * 16 + (CRTRegister[0xB] & 0x1F) * 2 + 2) * FullScalingY;
		crect.right = crect.left + (8 * FullScalingX);
...
static void VGA8_DrawCO80(HWND hwnd, unsigned char *mem)
{
	...
	displayFrameOnScreen(hdc, 0, 80, 640, 400, 640, 400, GFX640x480bmi, GFX640x480Bits, true);
...
static void MCGA_DrawMode11(HWND hwnd, unsigned char *mem)
{
	...
	displayFrameOnScreen(hdc, 0, 0, 640, 480, 640, 480, GFX640x480bmi, GFX640x480Bits, false);
...
{
	ScreenMode_t oldMode = CurrentScreenMode;
...
void CGA_UpdateFullScreenScaleRatio() {
	int fullWidth = GetSystemMetrics(SM_CXSCREEN);
	int fullHeight = GetSystemMetrics(SM_CYSCREEN);
	...
	if (FullScreen)
	{	
		double tempX = (double)fullWidth / (double)w;
		double tempY = (double)fullHeight / (double)h;

		if (FullScreenFit)
		{
			FullScalingX = min(tempX, tempY);
			FullScalingY = FullScalingX;
		}
		else
		{
			FullScalingX = tempX;
		...
		FullScalingXOffset = fullWidth - w * FullScalingX;
		FullScalingYOffset = fullHeight - h * FullScalingY;
	}
	else {
		FullScalingX = NormalScale;
		FullScalingY = NormalScale;
	}
	...
	GFXFullBmi.bmiHeader.biSize = sizeof(BITMAPINFOHEADER);
	GFXFullBmi.bmiHeader.biWidth = w * FullScalingX;
	GFXFullBmi.bmiHeader.biHeight = -h * FullScalingY;;
	GFXFullBmi.bmiHeader.biPlanes = 1;
	GFXFullBmi.bmiHeader.biBitCount = 24;
	GFXFullBmi.bmiHeader.biCompression = BI_RGB;
	GFXFullBmi.bmiHeader.biSizeImage = -GFXFullBmi.bmiHeader.biWidth * GFXFullBmi.bmiHeader.biHeight * 3;
	GFXFullBmi.bmiHeader.biXPelsPerMeter = 4096;
	GFXFullBmi.bmiHeader.biYPelsPerMeter = 4096;
	GFXFullBmi.bmiHeader.biClrUsed = 0;
	GFXFullBmi.bmiHeader.biClrImportant = 0;

	GFXFullBits = new char[GFXFullBmi.bmiHeader.biSizeImage];
}
...
void CGA_Initialise(void)
{
	...
	GFX320bmi.bmiHeader.biSizeImage = -GFX320bmi.bmiHeader.biWidth * GFX320bmi.bmiHeader.biHeight * 3;
	...
	GFX640bmi.bmiHeader.biSizeImage = -GFX640bmi.bmiHeader.biWidth * GFX640bmi.bmiHeader.biHeight * 3;
	...
	GFX640x480bmi.bmiHeader.biSizeImage = -GFX640x480bmi.bmiHeader.biWidth * GFX640x480bmi.bmiHeader.biHeight * 3;
...
void CGA_SetScale(double Scale)
{
	NormalScale = Scale;
...
void CGA_SetFullScreenAdjust(int x, int y) {
	FullScreenAdjustX = x;
	FullScreenAdjustY = y;
}

void CGA_SetFullScreenMethod(int method) {
	FSMethod = method;

}

void CGA_SetFullScreen(bool full) {
	FullScreen = full;
}

void CGA_SetFullScreenFit(bool fit) {
	FullScreenFit = fit;
}
...
void CGA_DrawScreen(HWND hwnd, unsigned char *mem)
{
	...
	if (needClearSurface)
	{
		CGA_ClearDisplaySurface(hwnd);
		needClearSurface = false;
	}
